...
insert into cliente values('AB568','estephano','calle 13 zona2','1432-1244','coban',11),
                          ...
                          ('DA236','alga','calle 15 zona 2','chamelco','1344-6532',13),
...
/*Listar el nombre de los clientes que no contienen en su nombre la cadena “GA”*/
select nombre from cliente where nombre not like'%ga%';

/*Mostrar nombre clientes agrupados por ciudad*/
select nombre from cliente group by ciudad;

/*Contar la cantidad de productos*/
select count(id_producto) from producto;

/*Muestre en pantalla los nombres de los clientes que No inician con A.*/
select nombre from cliente where nombre not like 'a%';

/*Listar las diferentes categorías en orden alfabético ascendente*/
select categoria from producto order by categoria asc;

/*Calcular la suma de las comisiones*/
select sum(comision) from venta;
